...
## Tooling Context: SD.Next Fork

The workload documented in this section was executed using **SD.Next**, a highly active fork of the Stable Diffusion WebUI that emphasizes performance, extended model support, and deeper access to backend execution paths.

SD.Next differs materially from baseline WebUI implementations in ways that matter for this investigation, including:
- Long-lived CUDA context management
- Aggressive use of modern schedulers and refiners
- Broader exposure of backend configuration and runtime behavior
- Faster adoption of new PyTorch, CUDA, and GPU features

These characteristics make SD.Next particularly effective for sustained, high-pressure inference workloads — and therefore well-suited for exposing post-workload runtime regressions that would not surface under lighter usage.

This investigation does **not** depend on undocumented behavior or custom modifications. All execution paths used are described in the official SD.Next documentation.

For readers who want to understand the execution environment, backend design, and architectural choices behind the fork, refer to the upstream documentation:

**SD.Next Documentation**  

<img width="256" height="297" alt="image" src="https://github.com/user-attachments/assets/b8bdc66b-7107-456b-94b3-4b507295ba57" />

https://vladmandic.github.io/sdnext-docs/

The findings that follow rely on a critical distinction established here:

> The defining signal in this case is **successful inference under extreme load**, followed by instability only after returning to routine desktop operations.

That distinction is explored in the next section.
